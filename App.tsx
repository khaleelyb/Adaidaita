...
const App: React.FC = () => {
  ...
  const startCall = async () => {
    ...
    rtc.onRemoteStream((stream) => {
      console.log('鉁� Remote stream received');
      if (isMounted.current) {
    ...
    try {
      ...
      if (isMounted.current) {
        ...
        console.log('鉁� Local stream started');
      }
    } catch (err: any) {
  ...
  if (!currentUser) {
    return <AuthModal onSuccess={() => {
      console.log('鉁� Login successful');
    }} />;
  }
